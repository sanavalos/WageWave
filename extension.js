...
function activate(context) {
  ...
  const startWageWave = async () => {
    ...
    if (hourlyRate) {
      const changeRate = await vscode.window.showInformationMessage('WageWave is already setted. Would you like to change values?', 'Yes', 'No');
      if (changeRate === 'Yes') {
  ...
  const editWageWave = async () => {
    if (!hourlyRate) {
      vscode.window.showInformationMessage('WageWave is not set. Use "Start WageWave" command to start it.');
      return;
    }
  ...
  let editDisposable = vscode.commands.registerCommand('wagewave.editWageWave', editWageWave);
  ...
  context.subscriptions.push(disposable, pauseDisposable, resumeDisposable, editDisposable);
}
